...
/**
 * LapController — detects completion of a lap of the racetrack.
 *
 * A lap completes when the car crosses the start/finish (checkpoint 0) having
 * visited every other checkpoint since it was armed (via setBiome). Lenient
 * about entry point and direction, so it just means you actually drove the loop.
 * Timing/UI live in RaceController; this only fires `onLap` on completion.
 */

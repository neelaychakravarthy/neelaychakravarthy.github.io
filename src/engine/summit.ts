...
// the secondary peak sits on the dome (summit-relative offset + base radius); the
// car can't drive into it — it's a no-go island in the drivable surface.
export const SUMMIT_PEAK_DX = 2;
export const SUMMIT_PEAK_DZ = -11;
export const SUMMIT_PEAK_R = 5.5;
...
export function summitHeight(x: number, z: number): number | null {
  ...
  // the secondary peak is a wall — block driving into its base
  const px = dx - SUMMIT_PEAK_DX;
  const pz = dz - SUMMIT_PEAK_DZ;
  if (px * px + pz * pz < SUMMIT_PEAK_R * SUMMIT_PEAK_R) return null;
  const t = d2 / r2;
